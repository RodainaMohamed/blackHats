var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
import { Router } from '@angular/router';
import { Component } from '@angular/core';
import { TopBusinessesService } from "./topBusinesses.service";
var TopBusinessesComponent = (function () {
    function TopBusinessesComponent(topBusinessesService, router) {
        this.topBusinessesService = topBusinessesService;
        this.router = router;
        this.businesses = [];
    }
    TopBusinessesComponent.prototype.ngOnInit = function () {
        var _this = this;
        this.topBusinessesService.getBusinesses()
            .subscribe(function (business) {
            _this.businesses = business;
        }, function (err) {
            switch (err.status) {
                case 404:
                    _this.router.navigateByUrl('/404-error');
                    break;
                case 401:
                    _this.router.navigateByUrl('/notAuthorized-error');
                    break;
                default:
                    _this.router.navigateByUrl('/500-error');
                    break;
            }
        });
    };
    return TopBusinessesComponent;
}());
TopBusinessesComponent = __decorate([
    Component({
        selector: 'homepage-topBusinesses',
        templateUrl: './topBusinesses.component.html',
        styleUrls: ['topBusinesses.component.css']
    }),
    __metadata("design:paramtypes", [TopBusinessesService,
        Router])
], TopBusinessesComponent);
export { TopBusinessesComponent };
